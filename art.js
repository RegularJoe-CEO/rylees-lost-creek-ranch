...
const ST=JSON.parse(localStorage.getItem('lcr2')||'{"stars":0,"done":{}}');
const save=()=>localStorage.setItem('lcr2',JSON.stringify(ST));
...
function acts(btns){$('acts').innerHTML=btns.map(b=>'<button class="btn '+(b.c||'')+'" onclick="('+b.fn+')()">'+b.t+'</button>').join('')}
function picks(opts,on){
 $('acts').innerHTML='';
 $('choice').classList.remove('hide');
 $('choice').innerHTML=opts.map((o,i)=>'<button class="pick" id="p'+i+'">'+o.t+'</button>').join('');
 opts.forEach((o,i)=>{$('p'+i).onclick=()=>{
  if(o.ok){$('p'+i).classList.add('ok');ST.stars++;save();stars();setTimeout(on,700)}
  else{$('p'+i).classList.add('no');say('Friend',o.hint||'Not that one, bud. Look again.')}
 };});
}
function clearUI(){$('talk').classList.add('hide');$('acts').innerHTML='';$('choice').classList.add('hide');$('choice').innerHTML=''}
function sky(dusk){
 return '<defs><linearGradient id="sky" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="'+(dusk?'#e39b5a':'#7ec4d6')+'"/><stop offset="1" stop-color="'+(dusk?'#f3d7a0':'#d7ec9c')+'"/></linearGradient><linearGradient id="water" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="#2e7a88"/><stop offset="1" stop-color="#1c4e58"/></linearGradient></defs><rect width="360" height="640" fill="url(#sky)"/><circle cx="292" cy="78" r="34" fill="#f4e27a"/>';
}
