(function (root) {
  ...
  function map() {
    ...
    mount('<section class="metric-scene"><img src="assets/chapter-creek-crossing.webp" alt="" class="scene-art"><div class="metric-intro"><span class="eyebrow dark">THE RANCH MEASUREMENT TRAIL</span><h1>Find your place. Keep exploring.</h1><p>Measure creek trails, stock the barn, and help Grandma find number patterns. Earlier place-value chapters stay open for review.</p>'
      + '<label for="metric-lesson">Where is school today?</label><select id="metric-lesson">' + lessons.slice(0, 8).map(([name], i) => `<option value="${i + 1}"${current === i + 1 ? ' selected' : ''}>Lesson ${i + 1}: ${escape(name)}</option>`).join('') + '</select>'
      + '<div class="metric-actions">' + navButton('Play today’s lesson', 'play', `data-lesson="${current}"`) + navButton('Earlier ranch chapters', 'exit') + '</div><p class="quiet">Choose any lesson to review or work ahead. No timers or date locks.</p></div></section>'
      + '<section class="metric-lesson-list"><h2>Review, today, and ahead</h2><p>Follow the order in your Module 2 book. The next-module trail is a warm-up, not a school lesson.</p><div class="metric-grid">' + rows + '</div></section>', 'Measurement trail');
